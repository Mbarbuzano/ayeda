...
// ─────────────────────────────────────────────────────────
//  f) QUICKSORT con pivote central  O(n log n) promedio
//     Pivote: elemento central del subrango [ini, fin].
//     Caso base: subsecuencias de tamaño ≤ 5 se ordenan
//     directamente con selectionSort sobre ese subrango.
// ─────────────────────────────────────────────────────────

// Selección restringida al subrango [ini, ini+len)
template <typename Key>
void selectionSortRange(staticSequence<Key>& seq, unsigned ini, unsigned len) {
  for (unsigned i = ini; i < ini + len - 1; ++i) {
    unsigned minIdx = i;
    for (unsigned j = i + 1; j < ini + len; ++j)
      if (seq[j] < seq[minIdx]) minIdx = j;
    if (minIdx != i) {
      Key tmp      = seq[i];
      seq[i]       = seq[minIdx];
...
template <typename Key>
void quickSortRec(staticSequence<Key>& seq, unsigned ini, unsigned fin) {
  unsigned len = fin - ini + 1;

  // Caso base: ≤ 5 elementos → selectionSort sobre el subrango
  if (len <= 5) {
    if (len > 1) {
      selectionSortRange(seq, ini, len);
    }
    return;
  }

  // Pivote: elemento central
  unsigned pivotIdx = (ini + fin) / 2;
  Key pivot = seq[pivotIdx];

  // Partición de Lomuto adaptada con índices explícitos
  int i = static_cast<int>(ini);
  int j = static_cast<int>(fin);

  while (i <= j) {
    while (seq[static_cast<unsigned>(i)] < pivot) ++i;
    while (seq[static_cast<unsigned>(j)] > pivot) --j;
    if (i <= j) {
      Key tmp                       = seq[static_cast<unsigned>(i)];
      seq[static_cast<unsigned>(i)] = seq[static_cast<unsigned>(j)];
      seq[static_cast<unsigned>(j)] = tmp;
      ++i; --j;
    }
  }

  // Recursión sobre las dos particiones
  if (static_cast<unsigned>(j) > ini)
    quickSortRec(seq, ini, static_cast<unsigned>(j));
  if (static_cast<unsigned>(i) < fin)
    quickSortRec(seq, static_cast<unsigned>(i), fin);
}

template <typename Key>
void quickSort(staticSequence<Key>& seq, unsigned ini, unsigned fin) {
  if (ini >= fin) return;
  quickSortRec(seq, ini, fin);
}
